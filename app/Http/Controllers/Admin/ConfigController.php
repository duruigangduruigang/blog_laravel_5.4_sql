...
class ConfigController extends Controller
{
    ...
    public function index()
    {
        ...
        foreach ($data as$k=> $v)
        {
            switch ($v->field_type){
                case 'input':
                    $data[$k]->_html = '<input type="text" class="lg" name="conf_content[]" value="'.$v->conf_tips.'">';
                    break;
                case 'textarea':
                    $data[$k]->_html = '<textarea type="text" class="lg" name="conf_content[]">'.$v->conf_tips.'</textarea>';
                    break;
                case 'radio':
                    //1|开启,0|关闭
